...
# %%
# Step 9: Display confusion matrices for all models in tabular form

# Convert confusion matrices into a DataFrame
cm_data = []

for model_name, cm in best_confusion_matrices.items():
    cm_flat = cm.flatten()  # Flatten the confusion matrix to one row
    cm_data.append({
        'Model': model_name,
        'True Negative': cm_flat[0],
        'False Positive': cm_flat[1],
        'False Negative': cm_flat[2],
        'True Positive': cm_flat[3]
    })

# Create a DataFrame to show confusion matrix results
cm_df = pd.DataFrame(cm_data)jm
...
# %%
# Calculate ROC AUC per fold for each model
roc_auc_per_fold = {}

for model_name in models:
    roc_auc_per_fold[model_name] = results[model_name]['ROC AUC']

# Add these ROC AUC scores to the results table
roc_auc_df = pd.DataFrame(roc_auc_per_fold)
roc_auc_df = roc_auc_df.round(3)  # Round to 3 decimal places

# Display ROC AUC scores
print("\nROC AUC per fold for each model:")
print(roc_auc_df)
