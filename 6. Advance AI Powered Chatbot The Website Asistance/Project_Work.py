...
# %%
# Phase 5: Inference with Saved MPT-7B Model + FAISS Index
...
# 📦 Load saved MPT-7B model and tokenizer
model_save_path = "./models/mosaicml/mpt-7b"
tokenizer = AutoTokenizer.from_pretrained(model_save_path)
model = AutoModelForCausalLM.from_pretrained(model_save_path)
...
# ✂️ Updated truncation function to handle MPT-7B's 2048 token limit
def truncate_prompt(tokenizer, context_chunks, base_template, question, max_tokens=2048):
    for k in range(len(context_chunks), 0, -1):
        partial_context = "\n".join(context_chunks[:k])
        prompt = base_template.format(context=partial_context, question=question)
        tokenized = tokenizer(prompt, return_tensors="pt", truncation=False)
...
def rag_chat_loop(
    ...
    top_k=3,  # lowered to 3 for safer prompt size
    max_new_tokens=300,
):
    ...
    while True:
        ...
        # Step 3: Format top-k chunks with topic/title
        top_k_texts = []
        ...
        outputs = lm_model.generate(
            ...
            num_beams=3,         # Beam search improves factuality
            early_stopping=True,
...
# %%
print('run the prvious one first it contain all code phase 5 and 6.')
